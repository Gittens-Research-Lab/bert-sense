...
class W2VNet(nn.Module):
  ...
  def forward(self, word_ids, context_ids, neg_ids, return_alpha=False, temp=1):
    ...
    alpha = torch.matmul(disamb_embs, context_emb2) # (b,k,1)
...
class BertCtxDataset(Dataset):
  def __init__(self, data_paths, vocab_path, params, load_all=False):
    self.data_lens = []
    total = 0
  ...
  def __getitem__(self, idx):
    if len(self.word) == 0:
      for data_path in self.data_paths:
        ...
        self.ctx.append(data['ctx'][:] if self.load_all else data['ctx'])
        #self.bert = self.data['bert'][:] if self.load_all else data['bert']
        self.bertctx.append(data['bert_ctx'][:] if self.load_all else data['bert_ctx'])
    sample = {}
    ...
    sample['bertctx'] = self.bertctx[split_idx][local_idx]
    return sample
...
if __name__ == "__main__":
  ...
  for i in range(2,5):
    data_path.append(os.path.join(SCRATCH, f"data/bert-sense/small_750k_{i}_bert_ctx_d5.h5"))
  vocab_path = os.path.join(SCRATCH, "data/bert-sense/vocab10_small_750k.txt")
  exp_dir = os.path.join(SCRATCH, "output/bert-sense", "small-750k-split-dissg-bert-ctx-d5-s3-d300-b2048-ns10-adam-lr1e3-sense-temp4-alpha2")
  ...
  #restore_file = os.path.join(SCRATCH, "output/bert-sense/", "westwiki-200m-vocab750k-d5-s3-d300-adam-lr1e3-ss0-b2048-ns10-sense-alpha2", "model-1.tar")

  bert_path = os.path.join(SCRATCH, "output/bert-sense", "small-750k-noshuf-bert-d5-s3-adam-lr1e3", "model-10.tar")
  #bert_path = os.path.join(SCRATCH, "output/bert-sense", "small-750k-split-bert-d5-s3-adam-lr1e3", "model-5.tar")
  logging.info(f"Bert weights: {bert_path}")
 
  params = Params('paramsw2vbert.json')
  with h5py.File(data_path[0], 'r', libver='latest', swmr=True) as data_file:
    d = {}
  ...
  logging.info("Dataset size: len(small_dataset)")
  ...
  logging.info("Loading Bert-sense model parameters from {}".format(bert_path))
